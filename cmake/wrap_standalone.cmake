function(target_add_standalone_wrapper)
  ...
  if(NOT DEFINED SA_TARGET)
    message(
      FATAL_ERROR
        "clap-wrapper: target_add_standalone_wrapper requires a target")
  endif()
  ...
  target_link_libraries(
    ${salib} PUBLIC clap-wrapper-compile-options clap-wrapper-shared-detail
                    base-sdk-rtmidi base-sdk-rtaudio)

  if(APPLE)
    target_sources(${salib} PRIVATE)
    target_link_libraries(
      ${salib} PUBLIC "-framework AVFoundation" "-framework Foundation"
  ...
  if(APPLE)
    set(MAIN_XIB
        "${CLAP_WRAPPER_CMAKE_CURRENT_SOURCE_DIR}/src/detail/standalone/macos/MainMenu.xib"
    )
    ...
    set_target_properties(
      ...
      PROPERTIES
        ...
        ${CLAP_WRAPPER_CMAKE_CURRENT_SOURCE_DIR}/src/detail/standalone/macos/Info.plist.in
        RESOURCE "${GEN_XIB}")

    if(NOT ${CMAKE_GENERATOR} STREQUAL "Xcode")
      message(
        STATUS
          "cmake-wrapper: ejecting xib->nib rules manually for ${CMAKE_GENERATOR} on ${SA_TARGET}"
      )
      ...
      add_custom_command(
        ...
        PRE_BUILD
        COMMAND
          ${CMAKE_COMMAND} -E echo ${IBTOOL} --compile
          "$<TARGET_FILE_DIR:${SA_TARGET}>/../Resources/MainMenu.nib" ${GEN_XIB}
        COMMAND
          ${IBTOOL} --compile
          "$<TARGET_FILE_DIR:${SA_TARGET}>/../Resources/MainMenu.nib"
          ${GEN_XIB})
    endif()

    macos_include_clap_in_bundle(
      TARGET ${SA_TARGET} MACOS_EMBEDDED_CLAP_LOCATION
      ${SA_MACOS_EMBEDDED_CLAP_LOCATION})
  ...
  elseif(WIN32 AND (CMAKE_CXX_COMPILER_ID MATCHES "MSVC"
    ...
    target_sources(
      ${SA_TARGET}
      PRIVATE
        ${CLAP_WRAPPER_CMAKE_CURRENT_SOURCE_DIR}/src/detail/standalone/windows/main.cpp
        ${CLAP_WRAPPER_CMAKE_CURRENT_SOURCE_DIR}/src/detail/standalone/windows/win32gui.cpp
        ...
        ${CLAP_WRAPPER_CMAKE_CURRENT_SOURCE_DIR}/src/detail/standalone/windows/win32gui.manifest
    )
  ...
  elseif(UNIX)
    target_sources(
      ${SA_TARGET}
      PRIVATE ${CLAP_WRAPPER_CMAKE_CURRENT_SOURCE_DIR}/src/wrapasstandalone.cpp)
    ...
    if(${GTK_FOUND})
      ...
      target_sources(
        ${salib}
        PRIVATE
          ${CLAP_WRAPPER_CMAKE_CURRENT_SOURCE_DIR}/src/detail/standalone/linux/gtkutils.cpp
      )
    else()
  ...
  target_link_libraries(${SA_TARGET} PRIVATE clap-wrapper-compile-options
                                             ${salib})
